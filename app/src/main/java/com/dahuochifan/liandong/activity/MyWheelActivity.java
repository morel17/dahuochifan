...
public class MyWheelActivity extends BaseLiandongActivity implements OnWheelChangedListener {
	...
	public void okFunction() {
		...
		if(mCurrentProviceName.equals("香港")||mCurrentProviceName.equals("澳门")||mCurrentProviceName.equals("台湾")){
			...
			if(!TextUtils.isEmpty(mCurrentCityName)&&!(mCurrentCityName.equals("----"))&&!(mCurrentCityName.equals("null"))){
				...
				MainTools.ShowToast(MyWheelActivity.this, "请选择市区");
				intent.putExtra("city", "");
				return;
			}
		}
...
//		overridePendingTransition(0, R.anim.fadeout);
	...
	private void setUpData() {
		initProvinceDatas();
		mViewProvince.setViewAdapter(new ArrayWheelAdapter<String>(MyWheelActivity.this, mProvinceDatas));
		// 设置可见条目数量
	...
	public void onChanged(WheelView wheel, int oldValue, int newValue) {
		 ...
		 * else if (wheel == mViewDistrict) { mCurrentDistrictName =
		 * mDistrictDatasMap.get(mCurrentCityName)[newValue]; mCurrentZipCode =
		 * mZipcodeDatasMap.get(mCurrentDistrictName); }
		 */
	}
	...
	private void updateAreas() {
		 ...
		 * areas)); mViewDistrict.setCurrentItem(0);
		 */
	}
