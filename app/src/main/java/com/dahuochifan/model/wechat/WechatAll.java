package com.dahuochifan.model.wechat;
...
public class WechatAll {
	...
	public boolean dealData(String jsonString, Activity activity) {
		...
		if (info2!=null&&info2.getResultCode()==1) {
			...
			MyConstant.user.setWechat(true);
			if(!TextUtils.isEmpty(MyConstant.user.getUserids())){
				spf_util.putUserId(editor, MyConstant.user.getWxuserid());
				MyConstant.user.setUserids(MyConstant.user.getWxuserid());
			}
			spf_util.putLogin(editor, true);
			MyConstant.user.setLogin(true);
			spf_util.putWxLogin(editor, true);
