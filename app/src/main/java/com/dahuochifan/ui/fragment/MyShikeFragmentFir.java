...
public class MyShikeFragmentFir extends BaseFragment implements OnRefreshListener2<ListView> {
    View view;
    ...
    public MyShikeFragmentFir instance;
    // 初始页数
    private int page = 1;
    // 页数size
    ...
    public MyShikeFragmentFir newInstance() {
        if (instance == null) {
            instance = new MyShikeFragmentFir();
        }
        return instance;
    }
    ...
    public void getShikeOne(boolean isRefresh) {
        if (BuildConfig.LEO_DEBUG) L.e("hehehe3");
        if (isRefresh) {
            page = 1;
        }
                ...
                if (data.getstatus(responseString, mActivity, gson) == 1) {
                    if (!isRefresh) {
                        if (data.getObj().getPage().getList() != null && data.getObj().getPage().getList().size() > 0) {
                            ...
                            ShikeInfo skinfo = new ShikeInfo();
                            list.add(skinfo);
